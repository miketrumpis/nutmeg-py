...
def gaussian_smooth_1d(a, fwhm=5.0, axes=(-1,), inplace=True):
    """Smooth an array by convolving repeatedly on different axes with
    Guassian kernels.
...
##     if inplace:
##         _fftn(a, axes=axes, inplace=True, shift=True)
##         # name-only assignment
##         b = a 
##     else:
##         # new array assignment
##         b = _fftn(a, axes=axes, inplace=False, shift=True)
    ...
    for n, s in enumerate(sigma):        
        ...
        if g is None:
            ...
            x = np.linspace(-d/2, d/2-1, d)
            g = np.exp(-(x**2)/(2*s**2)).astype('D')
            fft1(g, shift=True, inplace=True)
            __kernel_cache[(d,s)] = g
        ax_slice = null_slices[:]
        ax_slice[axes[n]] = slice(None)
        safe_b_slice = all_slices[:]
        safe_b_slice[axes[n]] = slice(0,d)
        _fftn(a[safe_b_slice], axes=(axes[n],), inplace=inplace, shift=True)
        a[safe_b_slice] *= g[ax_slice].real
        _ifftn(a[safe_b_slice], axes=(axes[n],), inplace=inplace, shift=True)
    n = len(sigma)
    a /= np.sqrt((2*np.pi)**n * np.prod(sigma**2))
    #_ifftn(b, axes=axes, inplace=True, shift=True)
